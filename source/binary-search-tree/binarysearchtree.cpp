...
template<class ItemType>
bool
BinarySearchTree<ItemType>::isEmpty() const
{
...
template<class ItemType>
int
BinarySearchTree<ItemType>::getHeight() const
{
  return treeHeight;
}
...
template<class ItemType>
int
BinarySearchTree<ItemType>::getNumberOfNodes() const
{
...
template<class ItemType>
bool
BinarySearchTree<ItemType>::add(const ItemType& item)
{
  ...
  if (isEmpty()) {
    ...
    rootPtr = newNodePtr;
    treeHeight = 1;
  } else {               // otherwise, add new item to root's child node.
...
template<class ItemType>
void
BinarySearchTree<ItemType>::clear()
{
  if (rootPtr != nullptr) {
...
BinarySearchTree<ItemType>::contains(const ItemType& item) const
{
  ...
  return findNode(rootPtr, item) != nullptr;
}
                                      ...
                                      BinaryNode<ItemType>* newNodePtr)
{
  ...
  if (subTreePtr != nullptr) {
    ...
    if (newNodePtr->getItem() <= subTreePtr->getItem()) { // left subtree
      BinaryNode<ItemType>* left = subTreePtr->getLeftChildPtr();

      if (left != nullptr) {  // non-empty left node, keep going down.
      ...
      if (right != nullptr) { // non-empty right node, keep going down.
        treeHeightCount += 1; // going down one more level,
                              // so the height goes up by 1.
...
template<class ItemType>
void
BinarySearchTree<ItemType>::inorderTraverse(void visit(ItemType&)) const
{
  if (isEmpty())
    return;
...
template<class ItemType>
void
BinarySearchTree<ItemType>::rebalance()
{
  if (isEmpty())
    return;
...
template<class ItemType>
bool
BinarySearchTree<ItemType>::readTree(ItemType arr[], int n)
{
  if (n > 0) {
...
template<class ItemType>
void
BinarySearchTree<ItemType>::readFullTree(ItemType arr[], int first, int last)
{
  /*
...
template<class ItemType>
void
BinarySearchTree<ItemType>::displaySideways() const
{
  sideways(rootPtr, 0);
}
...
template<class ItemType>
void
BinarySearchTree<ItemType>::sideways(BinaryNode<ItemType>* current,
                                     int level) const
{
  if (current != NULL) {
    level++;
...
template<class ItemType>
void
BinarySearchTree<ItemType>::deleteSubtree(BinaryNode<ItemType>* subTreePtr)
{
  if (subTreePtr == nullptr)
    return;
...
template<class ItemType>
void
BinarySearchTree<ItemType>::inorder(void visit(ItemType&),
...
template<class ItemType>
void
BinarySearchTree<ItemType>::addInOrderArray(ItemType& item)
{
...
template<class ItemType>
void
BinarySearchTree<ItemType>::addPreOrder(BinaryNode<ItemType>* subTreePtr)
{
  if (subTreePtr != nullptr) {
